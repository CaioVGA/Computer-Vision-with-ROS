...
def get_contour_center(contour):
    # Calculate moments from object. This will find a few of features, like mass center and area of an object.
...
def process_contours(binary, rgb_img, contours):
    # black_img = np.zeros(shape=(contours(binary.shape[0], binary.shape[1]),3), dtype=np.uint8)
    for cont in contours:
        area = cv2.contourArea(cont)
        # perimeter = cv2.arcLength(cont, True)
        perimeter = 0.1*cv2.arcLength(cont,True)
        # approx = cv2.approxPolyDP(cont,perimeter,True)
        ((x, y), radius) = cv2.minEnclosingCircle(cont)
        if (area > 200 and area < 300 and perimeter < 50):
...
def main():
    ...
    while True:
        ...
        draw_contours(frame, contornos, "RGB Image Contours")
        process_contours(binary=frame_binary, rgb_img=frame, contours=contornos)
